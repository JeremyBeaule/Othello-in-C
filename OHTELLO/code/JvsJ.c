#include "struct.h"
int joueurvsjoueur(int chargement){// int chargement correspond a si l'on charge ou non la partie, 0 = non
    ...
    switch (event.type) {
        ...
        case SDL_MOUSEBUTTONUP:
            if (event.button.button == SDL_BUTTON_LEFT) {
                ...
                if(fin==1){
                    End_game();
                        if (x>350 && x<550 && y>350 && y<550){ //il faut changer la zone de clique en dehors de la grille sinon ca click tout seul sur le bouton fin si on place le dernier bouton dans la zone
